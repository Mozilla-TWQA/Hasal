    ...
    def start_recording(self):
        ...
        with open(self.env.recording_log_fp, 'w') as self.fh:
            if platform.system().lower() == "windows":
                ...
                self.process = subprocess.Popen(["ffmpeg", "-f", "avfoundation", "-framerate", str(self.env.DEFAULT_VIDEO_RECORDING_FPS), "-video_size", str(self.env.DEFAULT_VIDEO_RECORDING_WIDTH) + "*" + str(self.env.DEFAULT_VIDEO_RECORDING_HEIGHT), "-i", get_mac_os_display_channel(), "-filter:v", "crop=" + str(self.env.DEFAULT_VIDEO_RECORDING_WIDTH) + ":" + str(self.env.DEFAULT_VIDEO_RECORDING_HEIGHT) + ":0:0", "-c:v", "libx264", "-r", str(self.env.DEFAULT_VIDEO_RECORDING_FPS), "-preset", "veryfast", "-g", "15", "-crf", "0", self.env.video_output_fp], bufsize=-1, stdout=self.fh, stderr=self.fh)
            else:
