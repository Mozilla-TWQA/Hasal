...
class SpeedIndexGenerator(object):
    ...
    def generate_sample_result(input_generator_name, input_sample_dict, input_sample_index):
        ...
        if SpeedIndexGenerator.SEARCH_TARGET_VIEWPORT in input_sample_data:
            return_result[input_generator_name]['crop_data'][SpeedIndexGenerator.SEARCH_TARGET_VIEWPORT] = input_sample_data[SpeedIndexGenerator.SEARCH_TARGET_VIEWPORT]
        else:
            viewport_value = find_image_viewport(input_sample_data['fp'])
            return_result[input_generator_name]['crop_data'][SpeedIndexGenerator.SEARCH_TARGET_VIEWPORT] = viewport_value
            return_result[SpeedIndexGenerator.SEARCH_TARGET_VIEWPORT] = viewport_value

        # generate tab_view crop area
        if SpeedIndexGenerator.SEARCH_TARGET_TAB_VIEW in input_sample_data:
            return_result[input_generator_name]['crop_data'][SpeedIndexGenerator.SEARCH_TARGET_TAB_VIEW] = input_sample_data[SpeedIndexGenerator.SEARCH_TARGET_TAB_VIEW]
        else:
            tabview_value = find_tab_view(input_sample_data['fp'], return_result[input_generator_name]['crop_data'][
                SpeedIndexGenerator.SEARCH_TARGET_VIEWPORT])
            return_result[input_generator_name]['crop_data'][SpeedIndexGenerator.SEARCH_TARGET_TAB_VIEW] = tabview_value
            return_result[SpeedIndexGenerator.SEARCH_TARGET_TAB_VIEW] = tabview_value

        # generate browser crop area
        if SpeedIndexGenerator.SEARCH_TARGET_BROWSER in input_sample_data:
            return_result[input_generator_name]['crop_data'][SpeedIndexGenerator.SEARCH_TARGET_BROWSER] = input_sample_data[SpeedIndexGenerator.SEARCH_TARGET_BROWSER]
        else:
            browser_view_value = find_browser_view(
                return_result[input_generator_name]['crop_data'][SpeedIndexGenerator.SEARCH_TARGET_VIEWPORT],
                return_result[input_generator_name]['crop_data'][SpeedIndexGenerator.SEARCH_TARGET_TAB_VIEW])
            return_result[input_generator_name]['crop_data'][SpeedIndexGenerator.SEARCH_TARGET_BROWSER] = browser_view_value
            return_result[SpeedIndexGenerator.SEARCH_TARGET_BROWSER] = browser_view_value
        ...
        if input_sample_index == 1:
            for event_obj in SpeedIndexGenerator.BROWSER_VISUAL_EVENT_POINTS['backward_search']:
            ...
            for event_obj in SpeedIndexGenerator.BROWSER_VISUAL_EVENT_POINTS['forward_search']:
                search_target_fp = crop_data_dict[event_obj['search_target']]['fp_list'][0]['output_fp']
                return_result[input_generator_name]['event_tags'][event_obj['event']] = convert_to_dct(search_target_fp, event_obj['fraction'])
